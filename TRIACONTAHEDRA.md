...
## 5G. The larger scales — rosettes, never a second solid

Jeff asked whether the normals go on meeting further out, at some larger scale. They
do, at many radii — and **they never build another triacontahedron.** That turns out to
be a sharp structural statement rather than a limitation of the patch.

Every equal-radius concurrence on Sun gen 3 was enumerated without assuming any radius,
then each radius was re-grouped to see how many faces it actually supports. **71
distinct radii**, all in `Z[φ]`. The result is completely lopsided:

| radius | largest group | makeup |
|---|---|---|
| **ρ** | **10** | **mixed — 5 thick + 5 thin** |
| ρ√5, 3ρ, ρφ³, ρ(φ³+2), … all 70 others | **5 at most** | **pure — all thick or all thin** |

And every one of those fives is a **rosette**: five *distinct* orientations sharing one
tilt about an axis. Checked at ρφ³ (2,087 groups), ρ√5 (245) and 3ρ (101) — 100% at
each, no exceptions.
...
### Powers of φ: only the cube

Testing `R = ρ·φ^k` directly is stark:

```
ρφ⁰ = 1.376382   max group 10     the triacontahedron
ρφ¹ = 2.227033   max group  1     nothing
ρφ² = 3.603415   max group  1     nothing
ρφ³ = 5.830447   max group  5     rosettes, 2087 of them on Sun gen 4
...
### Does this need an infinite patch? No — and not de Bruijn either

It was the natural worry, and it is settled. Generation 6 of the same seeds gives
patches far larger than any radius probed: **Pe5 gen 6** is 33,820 rhombi at radius
110, **Deca gen 6** is 218,930 at radius **354**, against a largest probed radius of
about 46. Rerunning every one of the observed radii there:

> **Every radius except ρ still caps at 5, and ρ is still the only mixed one.**
> 0 exceptions at patch radius 354.

So the cap is structural at every radius tested, not merely at the well-sampled ones,
and the sparse counts at large R on the small patches were indeed edge starvation and
nothing more.

**The de Bruijn plan is withdrawn, and Jeff's objection to it is the better reason.**
The pentagrid takes five offsets `γⱼ`, and the construction is regular — no three lines
concurrent — only for generic offsets. The tilings with exact five-fold symmetry are
precisely the *singular* pentagrids, where three lines do meet and the tiling is
ambiguous until a limiting or tie-breaking rule is imposed. Sun and Star are those two
tilings. So the straightforward generic recipe cannot produce them at all, and the
Queen is not a pentagrid tiling in the first place — it is a composite arrangement of
this project's own. A de Bruijn generator would hand back generic tilings, which is
exactly what this project is not about.

Growing the seed further gives arbitrarily large patches of **the actual three
