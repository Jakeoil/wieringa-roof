...
### Does this need an infinite patch?

For the well-sampled radii, **no** — and that is worth saying, because it was the
natural worry. Going from 2,440 rhombi to 16,475 left the cap at ρφ³ at exactly 5 while
multiplying the number of such groups by eight. The cap is structural, not
finite-size.

For the **large** radii it is the patch: at R ≈ 46 against a patch barely wider than
that, groups number 1 to 6 and are plainly edge-starved. Settling those does want an
unbounded tiling, generated by de Bruijn's pentagrid rather than grown from a seed, so
a window of any size can be cut anywhere. That is the case for building it.
